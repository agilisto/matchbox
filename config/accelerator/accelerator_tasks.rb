Capistrano::Configuration.instance(:must_exist).load do
    namespace :accelerator do
        ...
        desc "Adds a SMF for the application"
        task :create_daemon_smf, :roles => :app do

            template = File.read("config/opensolaris/daemon_smf_template.erb")
            buffer = ERB.new(template).result(binding)

            put buffer, "#{shared_path}/daemon-smf.xml"

            sudo "svccfg import #{shared_path}/daemon-smf.xml"
